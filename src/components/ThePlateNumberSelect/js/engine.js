(function (global, factory) {
  ...
  const Cached = {
    ...
    reg(layout, category, keys) {
      if (keys !== undefined && keys.constructor === Array) {
        const cached = this._mcached;
        keys.forEach((key) => {
          cached[(`${category }:${ key}`)] = layout;
        });
      } else {
        const keyIdx = (keys === undefined ? 0 : keys);
  ...
  const _STR_CIVIL_PVS = '京津晋冀蒙辽吉黑沪苏浙皖闽赣鲁豫鄂湘粤桂琼渝川贵云藏陕甘青宁新';
  const _STR_ARMY_PVS = 'QVKHBSLJNGCEZ';
  const _STR_ARMY_AREA = 'ABCDEFGHJKLMNOPRSTUVXY';
  const _STR_NUM = '1234567890';
  const _STR_LETTERS = 'QWERTYUPASDFGHJKLZXCVBNM';
  const _STR_CHARS = _STR_NUM + _STR_LETTERS;
  const _CHAR_EMBASSY = '使';
  const _CHAR_HK = '港';
  const _CHAR_MACAO = '澳';
  const _CHAR_XUE = '学';
  const _CHAR_JING = '警';
  const _CHAR_MIN = '民';
  const _CHAR_HANG = '航';
  const _STR_POSTFIX_ZH = `${_CHAR_JING}挂领试超`;
  const _CHAR_W = 'W';
  const _CHAR_J = 'J';
  const _CHAR_O = 'O';
  const _STR_DF = 'DF';
  const _STR_123 = '123';
  const _STR_NEW_ENERGY = _STR_123 + _STR_DF;
  const _STR_Q_IOP = 'QWERTYUIOP';
  const _STR_Q_OP = 'QWERTYUOP';
  const _STR_Q_P = 'QWERTYUP';
  const _STR_A_L = 'ASDFGHJKL';
  const _STR_Z_M = 'ZXCVBNM';
  const _STR_HK_MACAO = _CHAR_HK + _CHAR_MACAO;
  const _STR_EMBASSY_PVS = _CHAR_EMBASSY + _STR_123;
  const _CHAR_DEL = '-';
  const _STR_DEL_OK = `${_CHAR_DEL}+`;
  ...
  const _LAYOUT_CIVIL = 'layout.c';
  const _LAYOUT_WJ = 'layout.w';
  const _LAYOUT_WJ_FULL = 'layout.w.f';
  const _LAYOUT_FULL = 'layout.f';
  ...
  _GlobalConf.layoutProvider.reg((chain, args) => {
    if (args.index === 0 && args.keyboardType === KB_TYPES.CIVIL_WJ) {
  ...
  const _KEY_ANY = 'keys.any';
  const _KEY_CIVIL = 'keys.civil';
  const _KEY_ARMY = 'keys.army';
  const _KEY_WJ = 'keys.wj';
  const _KEY_AVIATION = 'keys.aviation';
  const _KEY_EMBASSY = 'keys.embassy';
  const _KEY_EMBASSY_ZH = 'keys.embassy.zh';
  const _KEY_NUMBRICS = 'keys.num';
  const _KEY_NUMBRICS_LETTERS = 'keys.num.letters';
  const _KEY_O_POLICE = 'keys.O.police';
  const _KEY_NUMERICS_DF = 'keys.num.df';
  const _KEY_HK_MACAO = 'keys.hk.macao';
  const _KEY_POSTFIX = 'keys.postfix';
  ...
  // 注册键位提供器，序号：0
  _GlobalConf.keyProvider.reg((chain, args) => {
    if (args.index === 0) {
  ...
  // 注册键位提供器，序号：7
  _GlobalConf.keyProvider.reg((chain, args) => {
    if (args.index === 7 && NUM_TYPES.NEW_ENERGY === args.numberType) {
  ...
  // 注册键位提供器，默认
  _GlobalConf.keyProvider.reg(() => Cached.load(_KEY_NUMBRICS_LETTERS));

  // //// 可用键位提供器 END //////

  function _rowOf(obj, index) {
    const data = obj[`row${index}`];
  ...
  // 注册键位可用性转换器
  _GlobalConf.mixiner.reg((layout, args) => {
    const availables = args.keys.map(ele => ele.text);
    return _mapLayout(layout, entity => _keyOfEnabled(entity, _in(availables, entity.text)));
  });

  // 禁用键位: 处理新能源键盘模式下，首位不允许出现的字符
  _GlobalConf.mixiner.reg((layout, args) => _mapLayout(layout, (entity) => {
  ...
  // 功能按钮的转换处理
  _GlobalConf.mixiner.reg(layout => _mapLayout(layout, (entity) => {
  ...
  // 处理删除键逻辑
  _GlobalConf.mixiner.reg(layout =>
    // 当输入车牌不为空时可以点击
    _mapLayout(layout, (entity) => {
  ...
  function __clone(srcObj) {
    const newCopy = srcObj.constructor();
  ...
  function _update(keyboardType, inputIndex, presetNumber, numberType) {
    ...
    const detectedNumberType = detectNumberTypeOf(presetNumber);
    ...
    const limitLength = NUM_TYPES.lenOf(presetNumberType);
    const presetLength = presetNumber.length;
    ...
    // 处理键位布局
    const output = __clone(_GlobalConf.layoutProvider.process(args));
    // 传递一些参数到外部
