  ...
  <div class="wx-shopDetail"
    ...
    <Banner class="banner-image">
      <!-- <img :src="`http://woxfile.oss-cn-shenzhen.aliyuncs.com/uploadfile/article/${shopInfo.entityPath ? shopInfo.entityPath : shopInfo.shopDefaultPic}`"
           alt=""
           slot="bannerImg"> -->
      <img :src="require('./image/default.jpg')"
           alt=""
           slot="bannerImg">
    </Banner>
    ...
    <div class="wx-shopBrief bg-white">
      ...
      <div class="wx-shopBrief__rightContent">
        ...
        <div class="wx-shopBrief__shopType background-theme">{{ shopInfo.code }}</div>
        <div class="wx-shopBrief__infoBrief">
          <div class="wx-shopBrief__address">
            地址： {{ shopInfo.floor }} {{ shopInfo.address ? '-' : '' }} {{ shopInfo.address }}
          </div>
          <div class="wx-shopBrief__fee"
               v-if="shopInfo.consumeValue">人均：￥{{ shopInfo.consumeValue }}/人</div>
        </div>

        <!-- <div class="wx-starsWrap">
          评价热度：
    ...
    <div class="bg-white act-detail wx-brandIntro m-model"
         v-if="shopInfo.remark">
      <h3 class="act-title color-theme m-title">品牌简介</h3>
      <MoreText :moreText="shopInfo.remark"></MoreText>
      <!-- <div class="act-content">
        {{ shopInfo.brandText }}
      </div> -->
    </div>
...
export default {
  ...
  async created() {
    const data = await service.shopIndexYL({
      code: this.$route.params.shopId,
      // name: this.$route.params.name,
    });

    this.shopInfo = data.Result[0];

    // if (data.Result.goodImgsArr) {
